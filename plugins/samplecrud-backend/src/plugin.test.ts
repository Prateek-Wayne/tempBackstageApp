...
import { samplecrudPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [samplecrudPlugin],
    });

    await request(server).get('/api/samplecrud/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/samplecrud/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/samplecrud/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/samplecrud/todos/${createdTodoItem.id}`)
